<template>
    <view class="bogbox">
                    ...
                    <div style="width:30vw;">
                        <el-form :model="form" label-width="120px">
                            ...
                            <el-form-item label="评论id">
                                <el-input v-model="form.reviewid" />
                            </el-form-item>
                            <el-button type="primary" plain style="margin-left: 8vw;" @click="createreviewlike">新建</el-button>
                        </el-form>
                    </div>
            </div>
        </view>
                    ...
                    <div style="width:30vw;">
                        <el-form :model="form2" label-width="120px">
                            ...
                            <el-form-item label="作者id">
                                <el-input v-model="form2.reviewid" />
                            </el-form-item>
                            <el-button type="primary" plain style="margin-left: 8vw;" @click="updatenewslike">更新</el-button>
                        </el-form>
                    </div>
            </div>
        </view>
        <!-- 大盒子 -->
        <div style="height:10vh;display:flex;flex-direction:row;align-items:center;">
                <el-button type="primary" style="margin-left:2vw;" @click="opencreate">添加评论点赞</el-button>
                <el-input v-model="Searchinput" placeholder="Please input" class="input-with-select" style="width: 60%;margin-left: 1vw;">
                    ...
                    <el-select v-model="Searchselect" placeholder="Select" style="width: 115px">
                        <el-option label="reviewlikeid" value="reviewlikeid" />
        ...
        <el-table :default-sort="sortrule" :data="tableData"  ref="multipleTableRef" style="width: 100%;height:60%;" :row-key="getRowKeys" ><!--@selection-change="handleSelectionChange" @selection-change="handleSelectionChange"-->
                ...
                <el-table-column fixed sortable  prop="reviewlikeid" label="reviewlikeid" width="300" />
                <el-table-column prop="userid" sortable label="用户id" width="300" />
                <el-table-column prop="reviewid" sortable label="评论id" width="300" />
                <el-table-column prop="createdAt" sortable label="创建日期" width="300" />
                <el-table-column fixed="right" label="Operations" width="300">
                    <template #default="scope"
                    ><!--scope 用来获取当前行的元素数据-->
                    <el-button link type="primary" size="small" @click="handleClick(scope.row.userid,scope.row.reviewid,scope.row.reviewlikeid)">编辑</el-button>
                    <el-button link type="primary" size="small" @click="deleteuser(scope.row.reviewlikeid)">删除</el-button>
                    </template>
                </el-table-column>
            </el-table>
...
let currentreviewlikeid;//当前collectsid
let currentPage4=ref(1)//当前分页
let total=ref(0)//数量总数
let pagesize=10;//分页每页显示10条数据
let Searchinput=ref()//搜索框
let Searchselect=ref('reviewlikeid')//选择框的值
let sortrule={ //排序规则
  prop: 'reviewlikeid', order: 'descending',
...
function searchnews(){//搜索事件 
    ...
    let userid=0;
    let reviewid=0;
    if(Searchselect.value=='reviewlikeid'){
        reviewlikeid=Searchinput.value
    }else if(Searchselect.value=='userid'){
        userid=Searchinput.value
    }else{
        reviewid=Searchinput.value
    }
    axios.post('/admin/seareviewlike',{reviewlikeid,userid,reviewid})
        .then((res)=>{
            console.log(res)
            alltableData.value=[]
            let i=0
...
function deleteuser(reviewlikeids){//删除关注
    let reviewlikeid=reviewlikeids
    axios.post('/admin/deletereviewlike',{reviewlikeid:`${reviewlikeid}`})
        .then((res)=>{
...
function updatenewslike(){//更新关注
    let reviewlikeid=currentreviewlikeid
    let userid=form2.userid
    let reviewid=form2.reviewid
    axios.post('/admin/updatereviewlike',{reviewlikeid:`${reviewlikeid}`,userid:`${userid}`,reviewid:`${reviewid}`})
    .then((res)=>{
...
function handleClick(userid,reviewid,reviewlikeid){//打开编辑界面
    updateflag.value=true
    form2.userid=userid
    form2.reviewid=reviewid
    currentreviewlikeid=reviewlikeid
}
function createreviewlike(){//创建新关注
    axios.post('/admin/createreviewlike',{userid:`${form.userid}`,reviewid:`${form.reviewid}`})
    .then((res)=>{
        if(res.data.msg=='已经存在'){ElMessage("已经存在")}
...
const getRowKeys = (row) => {
      //记录每行的key值
      return row.reviewlikeid;
}
function start() {//初始化
    let i=0;
    alltableData.value=[]
    tableData.value=[]
    axios.get("/admin/reviewlike")
        .then((res)=>{
            Object.entries(res.data.model).forEach((item)=>{
                i++
                alltableData.value.push({
                reviewlikeid:item[1].reviewlikeid,
                userid:item[1].userid,
                reviewid:item[1].reviewid,
